...
- MCP stdio calls are enforced through `agentfence mcp proxy`; HTTP JSON-RPC, batch JSON-RPC governance, GET/SSE stream requests, chunked JSON list filtering, and SSE list filtering are routed through `agentfence mcp http-proxy`.
