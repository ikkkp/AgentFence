...
Point the MCP client at `http://127.0.0.1:37422`. The proxy enforces POST JSON-RPC calls, governs batch JSON-RPC bodies without partial forwarding, filters denied entries from complete JSON, chunked JSON, batch, and SSE list responses, and passes other streaming responses through after request-level checks.
