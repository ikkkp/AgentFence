...
The desktop app lives in `apps/desktop`. It uses Tauri, React, TypeScript, and Vite. The current UI has dashboard, approval, audit, policy, MCP, skill, export, and settings surfaces. The policy editor validates JSON, shows a line-level diff against the last loaded or saved policy, and can apply daemon-generated audit suggestions into the editable policy before changes are saved. Settings can point the UI at a different local daemon endpoint and test local approval notifications; by default it checks daemon health through `http://127.0.0.1:37421/health`.
...
The initial MCP stdio proxy is available through `agentfence mcp proxy`, and a scoped HTTP JSON-RPC/SSE proxy is available through `agentfence mcp http-proxy`. They inspect client-to-server JSON-RPC calls for `tools/call`, `resources/read`, and `prompts/get`, then block denied or rate-limited requests before they reach the upstream server. They also track `tools/list`, `resources/list`, and `prompts/list` requests so denied entries can be filtered out of complete upstream JSON list responses. The HTTP proxy passes through GET/SSE and chunked streaming responses after request-level checks.

Guarded shell commands also extract URL-like arguments and common Git/SSH remotes, then evaluate those domains against `network` policy before execution. Future milestones should add full PTY interception, external tool broker adapters, deeper stream-aware MCP filtering, and optional OS-level or proxy-level network/filesystem controls.
