...
The initial MCP stdio proxy is available through `agentfence mcp proxy`, and a scoped HTTP JSON-RPC/SSE proxy is available through `agentfence mcp http-proxy`. They inspect client-to-server JSON-RPC calls for `tools/call`, `resources/read`, and `prompts/get`, including batch arrays, then block denied or rate-limited requests before they reach the upstream server. If any controlled call in a batch fails policy, approval, or rate-limit checks, the whole batch is rejected rather than partially forwarded. They also track `tools/list`, `resources/list`, and `prompts/list` requests by JSON-RPC id so denied entries can be filtered out of complete upstream JSON list responses, chunked JSON list responses, batch responses, and JSON-RPC responses carried in SSE `data:` events. The HTTP proxy passes through GET/SSE and other streaming responses after request-level checks.
