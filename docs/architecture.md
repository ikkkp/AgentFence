...
The initial MCP stdio proxy is available through `agentfence mcp proxy`, and a scoped HTTP JSON-RPC/SSE proxy is available through `agentfence mcp http-proxy`. They inspect client-to-server JSON-RPC calls for `tools/call`, `resources/read`, and `prompts/get`, then block denied or rate-limited requests before they reach the upstream server. They also track `tools/list`, `resources/list`, and `prompts/list` requests so denied entries can be filtered out of complete upstream JSON list responses and JSON-RPC responses carried in SSE `data:` events. The HTTP proxy passes through GET/SSE and chunked streaming responses after request-level checks.

Guarded shell commands also extract URL-like arguments and common Git/SSH remotes, then evaluate those domains against `network` policy before execution. Future milestones should add full PTY interception, external tool broker adapters, non-SSE stream-aware MCP filtering, and optional OS-level or proxy-level network/filesystem controls.
