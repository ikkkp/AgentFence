...
- Milestone 3 MCP proxy: stdio and scoped HTTP JSON-RPC/SSE proxy enforcement for `tools/call`, `resources/read`, and `prompts/get`, batch JSON-RPC governance without partial forwarding, plus list filtering for complete JSON, chunked JSON, batch, and SSE responses, daemon-backed ask mode, rate limits, and audit events.
...
- Add hosted or remote team policy distribution for signed artifacts.
